...
def extract_bronchial(ct_slice, xy_spacing, BRONCHIAL_THRESHOLD=-950):
    ...
    Args:
        ct_slice (list[int]): a label of CT;
        xy_spacing (float): ???? ;
        BRONCHIAL_THRESHOLD (int): an average HU in the bronchi.

    Returns:
        list[int]: labeled - a label of the segmented bronchi and trachea on CT
...
def select_bronchial(bronchial, ct_slices, levels):
    ...
    Args:
        bronchial (list[int]): a label of the pixels with the bronchi;
        ct_slices (list[int]): a label of CT;
...
def select_seeds(bronchial, ct_clice):
    ...
    Args:
        bronchial (list[int]): a label of the pixels with the bronchi;
        ct_clice (list[int]): a label of CT;
...
def extract_seeds(patient, xy_spacing):
    ...
    Returns:
        list[int]: seeds - a initial region.
...
def growing_bronchis(patient, seeds,
    ...
    Returns:
        list[int]: ret - a label of the bronchi at the previous stage.
        list[int]: seeds - a label of the bronchi.
...
def grow_lungs(patient, seeds):
    ...
    Returns:
        list[int]: a label of the segmented lungs.
...
def separate(lungs_in):
    ...
    Args:
        lungs_in (list[int]): a slice with one connected component.

    Returns:
        list[int]: labeled - a label of the segmented lungs;
        list[int]: markers;
...
def lung_separation_two(lungs_seeds):
    ...
    Args:
        lungs_seeds (list[int]): a label of the lungs.

    Returns:
        list[int]: lung_new - right and left lungs.
        list[int]: right_lung;
        list[int]: left_lung.
...
def lungs_postprocessing(patient, lungs_seeds):
    ...
    Args:
        lungs_seeds (list[int]): a label with the segmented lungs before postprocessing

    Returns:
        list[int]: a label with the segmented lungs after binary_fill_holes
...
def flip_lung(patient, trachea):
    ...
    Args:
        patient(list[int]): a label of CT
        trachea (list[int]): a label of the trachea

    Returns:
        list[int]: patient - converted Lungs
    """
...
def conventional_lung_segmentation(patient, xy_spacing):
    ...
    Returns:
        list[int]: lungs_seeds - a label with the segmented lungs.
        list[int]: left - a label with the segmented left lung.
        list[int]: right - a label with the segmented right lung.
    """
...
def cumulation(lung):
    ...
    Args:
        lung (list[int]): The segmented lung (left or right)
...
def ventricular_extraction(err_lung):
    ...
    Args:
        err_lung (list[int]): the label of errors in the lung.

    Returns:
        list[int]: err_lung - the label of errors in the lung without the ventricle.
...
def costal_surface(lung, z_coords, max_coor, combined):
    ...
    Args:
        lung (list[int]): cumulative x-position in lung
        z_coords ??? : z-coordinates in lung
        max_coor ??? : threshold for the сostal surface for the lung
        combined (list[int]): segmented lung (left or right)

    Returns:
        lung, erroneus, erroneus_erosion
    ...
    erroneus_erosion = scipy.ndimage.morphology.binary_dilation(erroneus_erosion, structure=skimage.morphology.ball(4.5))

    return lung, erroneus, erroneus_erosion


def detection_lung_error(patient, combined):
    """ Automatic error detection.

    The function aggregates cumulation, costal_surface, ventricular_extraction.
    Args:
        patient ????: file .mhd
        combined ????: a mask of the segmented lung

    Returns:
        list[int]: the masks of error in the lungs
    """
    ...
    lung_left = ((combined.copy() == 2)
                 ...
                 | (combined.copy() == 10)
                 | (combined.copy() == 12)
                 | (combined.copy() == 14)).astype(float)

    lung_right_comm, z_coords_r = cumulation(lung_right)
    lung_left_comm, z_coords_l = cumulation(lung_left)

    lung_left, er_l, erroneus_erosion_left = costal_surface(lung_left_comm, z_coords_l, .2, lung_left)
    lung_right, er_r, erroneus_erosion_right = costal_surface(lung_right_comm, z_coords_r, .8, lung_right)
